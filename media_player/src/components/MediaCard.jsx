...
import { Link } from 'react-router'; // Using react-router-dom for modern projects
...
const MediaCard = ({ media }) => {
    return (
        <Link to={`/watch/${media.id}`} key={media.id} className="block group">
            <div className="bg-slate-800 rounded-lg shadow-lg overflow-hidden transform transition-transform duration-200 hover:scale-105 hover:shadow-2xl">
                ...
                <div className="p-4">
                    {/* Media title */}
                    <h3 className="text-xl font-semibold text-slate-400 mb-2 truncate">
                        {media.title}
                    </h3>
